...
abstract class TweetSet {
	...
	def union(that : TweetSet) : TweetSet = this.filterAcc(tw => true, that.filter(tw => true))
...
class Empty extends TweetSet {
	...
	def mostRetweeted : Tweet = throw new NoSuchElementException("Empty set contains nothing")
...
class NonEmpty(elem : Tweet, left : TweetSet, right : TweetSet) extends TweetSet {
	...
	def mostRetweeted : Tweet = {
		val leftMax = if (!left.isInstanceOf[Empty]) left.mostRetweeted else elem
		val rightMax = if (!right.isInstanceOf[Empty]) right.mostRetweeted else elem
	...
	def descendingByRetweet : TweetList = {
		descendingByRetweetInt(this, Nil)
	}

	def descendingByRetweetInt(set: TweetSet, acc: TweetList) : TweetList = {
		if (set.isInstanceOf[Empty]) acc
		else {
			val maxTweet = set.mostRetweeted
			new Cons(maxTweet, descendingByRetweetInt(set.remove(maxTweet), acc))
		}
	}
...
object GoogleVsApple {
	...
	def predicate(words: List[String]) = (tx: Tweet) => {
		words.exists((word: String) => {
			tx.text.contains(word)
		})
	}
